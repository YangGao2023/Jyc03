...
function ClientsSection({ clients, setClients, suppliers, setSuppliers }: { clients: ContactRecord[]; setClients: React.Dispatch<React.SetStateAction<ContactRecord[]>>; suppliers: SupplierRecord[]; setSuppliers: React.Dispatch<React.SetStateAction<SupplierRecord[]>>; }) {
  ...
  function exportContacts() { if (sub === "clients") { downloadCsv(`biz-clients-${todayIso()}.csv`, [["客户名称", "联系人", "电话", "邮箱/微信", "地址", "创建日期", "备注", "VIP", "欠款"], ...clients.map((item) => [item.name, item.contact ?? "", item.phone ?? "", item.email ?? item.wechat ?? "", item.address ?? "", item.created_at ?? "", item.note ?? "", item.is_vip ? "是" : "否", item.balance ?? 0])]); return; } downloadCsv(`biz-suppliers-${todayIso()}.csv`, [["供应商名称", "类别", "联系人", "电话", "地址", "最近采购日期", "备注"], ...suppliers.map((item) => [item.name, item.category ?? "", item.contact_person ?? "", item.phone ?? "", item.address ?? "", item.last_purchase_date ?? "", item.remark ?? ""])]); }
  function printContacts() { if (sub === "clients") { openPrintWindow(buildSimpleTablePrintHTML("客户档案", `共 ${clients.length} 条`, ["客户名称", "联系人", "电话", "邮箱/微信", "地址", "创建日期", "备注"], clients.map((item) => [item.name, item.contact ?? "-", item.phone ?? "-", item.email ?? item.wechat ?? "-", item.address ?? "-", item.created_at ?? "-", item.note ?? "-"]))); return; } openPrintWindow(buildSimpleTablePrintHTML("供应商清单", `共 ${suppliers.length} 条`, ["供应商名称", "类别", "联系人", "电话", "地址", "最近采购日期", "备注"], suppliers.map((item) => [item.name, item.category ?? "-", item.contact_person ?? "-", item.phone ?? "-", item.address ?? "-", item.last_purchase_date ?? "-", item.remark ?? "-"]))); }
