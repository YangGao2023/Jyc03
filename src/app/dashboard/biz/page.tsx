...
function getOverviewDomains(orderSummary: ReturnType<typeof summarizeOrders>): Array<{
  key: string;
  title: string;
  sub: string;
  color: string;
  ...
  const totalExpense = bizExpenses.reduce((sum, item) => sum + item.amount, 0);
  const totalPayroll = bizPayrolls.reduce((sum, item) => sum + item.net_salary, 0);
  const draftQuotes = bizQuotes.filter((item) => item.status === "草稿").length;
  const sentQuotes = bizQuotes.filter((item) => item.status === "已发送").length;
  const wonQuotes = bizQuotes.filter((item) => item.status === "已成交").length;
  const lowStockCount = bizMaterials.filter((item) => item.stock_quantity <= item.min_stock).length;
  const activeEmployees = bizEmployees.filter((item) => item.status === "在职").length;
  const monthlyPayroll = bizPayrolls
  ...
  return [
    ...
    },
    {
      ...
      dot: "bg-sky-500",
      stats: [
        { label: "客户", value: String(bizClients.length) },
        { label: "供应商", value: String(bizSuppliers.length) },
      ],
    },
    {
      key: "materials",
      ...
      dot: "bg-amber-500",
      stats: [
        { label: "品类", value: String(bizMaterials.length) },
...
function OverviewSection({
  ...
  const domains = getOverviewDomains(orderSummary);
  return (
    <div>
...
5. Warranty depends on the size and style.`)
  ...
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Invoice ${escHtml(order.order_number)}</title>
<style>
*{margin:0;padding:0;box-sizing:border-box}
@page{size:A4 portrait;margin:4mm}
html,body{margin:0;padding:0;background:#fff}
body{font-family:Arial,sans-serif;color:#111827;padding:0}
.sheet{width:202mm;height:289mm;overflow:hidden;margin:0 auto;border:1px solid #a3a3a3;background:#fff}
.topbar{display:grid;grid-template-columns:1.2fr 1fr;gap:4px;padding:4px;background:${brandBlue}}
.topbox{background:${brandBlue};color:#fff;padding:10px 12px;min-height:52px;display:flex;align-items:center;justify-content:space-between;border:1px solid rgba(255,255,255,.35)}
.topbox h1{font-size:14px;font-weight:800;letter-spacing:.02em;color:#fff}
.topbox p{font-size:10px;line-height:1.2;color:rgba(255,255,255,.92)}
.sectionBlue{background:${brandBlue};color:#fff;font-weight:700;padding:7px 10px;font-size:13px}
.contactInvoice{display:grid;grid-template-columns:2.4fr 1fr;gap:4px;padding:0 4px 4px;background:${brandBlue}}
.panel{border:1px solid #a3a3a3;background:#fff}
.rows{padding:0;background:#fff}
.row{display:grid;grid-template-columns:58px 1fr 58px 1fr;border-top:1px solid #d4d4d4}
.row:first-child{border-top:none}
.cell{padding:8px 10px;font-size:11px}
.label{font-weight:700}
.invoiceBox .big{font-size:16px;font-weight:800;text-align:center;padding:14px 10px;border-top:1px solid #d4d4d4}
.invoiceBox .date{font-size:14px;font-weight:800;text-align:center;padding:14px 10px;border-top:1px solid #d4d4d4}
.midBlue{margin:0 4px 4px;background:${brandBlue};color:#fff;padding:7px 10px;font-size:11px;font-weight:700;display:flex;justify-content:space-between}
.middle{display:grid;grid-template-columns:1.08fr 0.92fr;gap:4px;padding:0 4px 4px}
.photoWrap{border:1px solid #a3a3a3;background:#fff;height:148mm;position:relative;padding:4px}
.photoInner{height:100%;border:1px solid #111827;background:#e5e7eb;overflow:hidden}
.photoCode{position:absolute;left:12px;bottom:8px;font-size:24px;font-weight:900;letter-spacing:.04em;color:#111}
.descWrap{border:1px solid #a3a3a3;background:#fff;height:148mm;overflow:hidden}
.descWrap > div{padding:10px!important;font-size:12px!important;line-height:1.45!important;min-height:auto!important}
.bottom{display:grid;grid-template-columns:1.2fr .9fr;gap:4px;padding:0 4px 4px}
.noteBody{border:1px solid #a3a3a3;background:#fff;padding:10px;min-height:72mm;font-size:11px;line-height:1.45;overflow:hidden}
.totalBody{border:1px solid #a3a3a3;background:#fff;padding:10px 12px;min-height:72mm;display:flex;flex-direction:column;justify-content:center;gap:10px}
.totalRow{display:flex;justify-content:space-between;font-size:15px;font-weight:800}
.totalRow .v{color:#2b6fdb}
@media print{html,body{margin:0!important;padding:0!important}.sheet{border:none;-webkit-print-color-adjust:exact;print-color-adjust:exact;break-inside:avoid;page-break-inside:avoid}}
</style></head>
<body>
<div class="sheet">
  <div class="topbar">
...
function ClientsSection({ clients, setClients, suppliers, setSuppliers }: { clients: ContactRecord[]; setClients: React.Dispatch<React.SetStateAction<ContactRecord[]>>; suppliers: SupplierRecord[]; setSuppliers: React.Dispatch<React.SetStateAction<SupplierRecord[]>>; }) {
  ...
  function addClient() { if (!clientDraft.name.trim()) return; setClients((prev) => [{ id: `CL-${String(prev.length + 1).padStart(3, "0")}`, name: clientDraft.name.trim(), contact: clientDraft.contact || undefined, phone: clientDraft.phone || undefined, wechat: clientDraft.wechat || undefined, address: clientDraft.address || undefined, note: clientDraft.note || undefined, created_at: today, balance: 0, is_vip: false }, ...prev]); setClientDraft({ name: "", contact: "", phone: "", wechat: "", address: "", note: "" }); }
  function addSupplier() { if (!supplierDraft.name.trim()) return; setSuppliers((prev) => [{ id: `SUP-${String(prev.length + 1).padStart(3, "0")}`, name: supplierDraft.name.trim(), category: supplierDraft.category, contact_person: supplierDraft.contact_person || undefined, phone: supplierDraft.phone || undefined, address: supplierDraft.address || undefined, remark: supplierDraft.remark || undefined, last_purchase_date: today }, ...prev]); setSupplierDraft({ name: "", category: "布料", contact_person: "", phone: "", address: "", remark: "" }); }
  return <div><SectionHeader eyebrow="Contacts" title="客户 & 供应商" actions={<ActionBtn tone="primary" onClick={sub === "clients" ? addClient : addSupplier}>+ 新建{sub === "clients" ? "客户" : "供应商"}</ActionBtn>} /><StatStrip items={[{ label: "客户总数", value: String(clients.length) }, { label: "VIP 客户", value: String(clients.filter((item) => item.is_vip).length), accent: "text-sky-600" }, { label: "供应商", value: String(suppliers.length) }, { label: "欠款客户", value: String(clients.filter((item) => (item.balance ?? 0) > 0).length), accent: "text-amber-600" }]} /><SegmentedControl options={[{ key: "clients", label: "客户档案" }, { key: "suppliers", label: "供应商" }]} value={sub} onChange={setSub} />{sub === "clients" ? <div className="space-y-4"><PanelCard title="快速录入客户"><div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3"><SmallInput value={clientDraft.name} onChange={(v) => setClientDraft((d) => ({ ...d, name: v }))} placeholder="客户名称" /><SmallInput value={clientDraft.contact} onChange={(v) => setClientDraft((d) => ({ ...d, contact: v }))} placeholder="联系人" /><SmallInput value={clientDraft.phone} onChange={(v) => setClientDraft((d) => ({ ...d, phone: v }))} placeholder="电话" /><SmallInput value={clientDraft.wechat} onChange={(v) => setClientDraft((d) => ({ ...d, wechat: v }))} placeholder="微信 / 邮箱" /><SmallInput value={clientDraft.address} onChange={(v) => setClientDraft((d) => ({ ...d, address: v }))} placeholder="地址" /><SmallInput value={clientDraft.note} onChange={(v) => setClientDraft((d) => ({ ...d, note: v }))} placeholder="备注" /></div></PanelCard><div className="overflow-x-auto rounded-xl border border-slate-200 bg-white"><table className="w-full text-left text-xs"><thead><tr className="border-b border-slate-200 bg-slate-50"><th className="px-4 py-2.5 font-semibold text-slate-600">客户名称</th><th className="px-4 py-2.5 font-semibold text-slate-600">联系人</th><th className="px-4 py-2.5 font-semibold text-slate-600">电话</th><th className="px-4 py-2.5 font-semibold text-slate-600">邮箱 / 微信</th><th className="px-4 py-2.5 font-semibold text-slate-600">地址</th><th className="px-4 py-2.5 font-semibold text-slate-600">创建日期</th><th className="px-4 py-2.5 font-semibold text-slate-600">备注</th></tr></thead><tbody>{clients.map((item) => <tr key={item.id} className="border-b border-slate-100 last:border-b-0"><td className="px-4 py-2.5 font-medium text-slate-700">{item.name}{item.is_vip ? <span className="ml-2 rounded-full bg-sky-50 px-2 py-0.5 text-[11px] font-semibold text-sky-700">VIP</span> : null}</td><td className="px-4 py-2.5 text-slate-600">{item.contact ?? "-"}</td><td className="px-4 py-2.5 text-slate-600">{item.phone ?? "-"}</td><td className="px-4 py-2.5 text-slate-600">{item.email ?? item.wechat ?? "-"}</td><td className="px-4 py-2.5 text-slate-500">{item.address ?? "-"}</td><td className="px-4 py-2.5 text-slate-500">{item.created_at ?? "-"}</td><td className="px-4 py-2.5 text-slate-500">{item.note ?? "-"}</td></tr>)}</tbody></table></div></div> : <div className="space-y-4"><PanelCard title="快速录入供应商"><div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3"><SmallInput value={supplierDraft.name} onChange={(v) => setSupplierDraft((d) => ({ ...d, name: v }))} placeholder="供应商名称" /><SmallSelect value={supplierDraft.category} onChange={(v) => setSupplierDraft((d) => ({ ...d, category: v }))} options={["布料", "轨道五金", "玻璃", "物流", "其他"]} /><SmallInput value={supplierDraft.contact_person} onChange={(v) => setSupplierDraft((d) => ({ ...d, contact_person: v }))} placeholder="联系人" /><SmallInput value={supplierDraft.phone} onChange={(v) => setSupplierDraft((d) => ({ ...d, phone: v }))} placeholder="电话" /><SmallInput value={supplierDraft.address} onChange={(v) => setSupplierDraft((d) => ({ ...d, address: v }))} placeholder="地址" /><SmallInput value={supplierDraft.remark} onChange={(v) => setSupplierDraft((d) => ({ ...d, remark: v }))} placeholder="备注" /></div></PanelCard><div className="overflow-x-auto rounded-xl border border-slate-200 bg-white"><table className="w-full text-left text-xs"><thead><tr className="border-b border-slate-200 bg-slate-50"><th className="px-4 py-2.5 font-semibold text-slate-600">供应商名称</th><th className="px-4 py-2.5 font-semibold text-slate-600">类别</th><th className="px-4 py-2.5 font-semibold text-slate-600">联系人</th><th className="px-4 py-2.5 font-semibold text-slate-600">电话</th><th className="px-4 py-2.5 font-semibold text-slate-600">地址</th><th className="px-4 py-2.5 font-semibold text-slate-600">最近采购日期</th><th className="px-4 py-2.5 font-semibold text-slate-600">备注</th></tr></thead><tbody>{suppliers.map((item) => <tr key={item.id} className="border-b border-slate-100 last:border-b-0"><td className="px-4 py-2.5 font-medium text-slate-700">{item.name}</td><td className="px-4 py-2.5 text-slate-600">{item.category ?? "-"}</td><td className="px-4 py-2.5 text-slate-600">{item.contact_person ?? "-"}</td><td className="px-4 py-2.5 text-slate-600">{item.phone ?? "-"}</td><td className="px-4 py-2.5 text-slate-500">{item.address ?? "-"}</td><td className="px-4 py-2.5 text-slate-500">{item.last_purchase_date ?? "-"}</td><td className="px-4 py-2.5 text-slate-500">{item.remark ?? "-"}</td></tr>)}</tbody></table></div></div>}</div>;
}
...
function QuotesSection({ quotes, setQuotes, showcases, setShowcases }: { quotes: QuoteRecord[]; setQuotes: React.Dispatch<React.SetStateAction<QuoteRecord[]>>; showcases: ShowcaseRecord[]; setShowcases: React.Dispatch<React.SetStateAction<ShowcaseRecord[]>>; }) {
