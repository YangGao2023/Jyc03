    ...
    <div className="rounded-[30px] border border-white/10 bg-[linear-gradient(180deg,_rgba(15,23,42,0.98),_rgba(3,7,18,0.98))] p-4 shadow-2xl">
          <div className="rounded-[24px] border border-white/10 bg-white/5 p-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="text-[11px] uppercase tracking-[0.28em] text-sky-300">Owner Backend · System</p>
                <h1 className="mt-2 text-2xl font-semibold text-white">系统页</h1>
                <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-300">这一页只看系统信号，不把任务、记忆、原型展示全挤进来。重点是在线状态、来源解释和最近事件。</p>
              </div>
          ...
          <div className="mt-4 grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
            <section className="rounded-[24px] bg-white p-4 text-slate-900 shadow-sm">
              <div className="flex items-center justify-between gap-3">
                <p className="text-sm font-medium text-sky-700">状态信号</p>
            ...
            <section className="rounded-[24px] bg-white p-4 text-slate-900 shadow-sm">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-500">从共享事件流读取最近系统相关动作</p>
                </div>
                <div className="flex items-center gap-2">
                  <span className="rounded-full bg-slate-900 px-3 py-1 text-xs font-semibold text-white">{events.length} / {allEvents.length} 条</span>
                  <a href="/dashboard?section=system" className="rounded-full border border-slate-200 px-3 py-1 text-xs font-semibold text-slate-700">旧版 system 面板</a>
                </div>
              </div>
