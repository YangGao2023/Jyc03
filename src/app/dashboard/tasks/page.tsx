...
function statusTone(value: string) {
  ...
  if (["in_progress", "进行中"].includes(normalized)) return "bg-sky-100 text-blue-200";
  ...
  if (["dropped", "已放弃"].includes(normalized)) return "bg-blue-800/40 text-blue-200";
  ...
  return (
    <div className="rounded-[30px] border border-blue-800/40 bg-[linear-gradient(180deg,_rgba(30,64,175,0.98),_rgba(23,37,84,0.98))] p-4 shadow-2xl">
          <div className="rounded-[24px] border border-blue-800/40 bg-blue-900/20 p-4">
            ...
            <form action="/api/task-create" method="POST" className="mt-4 rounded-2xl border border-blue-800/40 bg-blue-900/20 p-4">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-xs text-blue-200">直接从任务台写入任务队列和事件流。</p>
                </div>
              </div>
              <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                <label className="text-xs font-medium text-blue-200">
                  标题
                  <input name="title" className="mt-1 w-full rounded-xl border border-blue-800/40 bg-blue-950/60 px-3 py-2 text-sm text-white outline-none" placeholder="比如：接真实产品照片到前台" />
                </label>
                <label className="text-xs font-medium text-blue-200">
                  负责人
                  <select name="owner" defaultValue="待定" className="mt-1 w-full rounded-xl border border-blue-800/40 bg-blue-950/60 px-3 py-2 text-sm text-white outline-none">
                ...
                <label className="text-xs font-medium text-blue-200">
                  优先级
                  <select name="priority" defaultValue="P2" className="mt-1 w-full rounded-xl border border-blue-800/40 bg-blue-950/60 px-3 py-2 text-sm text-white outline-none">
                ...
                <label className="text-xs font-medium text-blue-200">
                  来源
                  <input name="source" defaultValue="老板后台新增任务" className="mt-1 w-full rounded-xl border border-blue-800/40 bg-blue-950/60 px-3 py-2 text-sm text-white outline-none" />
                </label>
              </div>
              <div className="mt-3 grid gap-3 md:grid-cols-[1.3fr_0.7fr]">
                <label className="text-xs font-medium text-blue-200">
                  下一步
                  <input name="nextAction" defaultValue="待补充" className="mt-1 w-full rounded-xl border border-blue-800/40 bg-blue-950/60 px-3 py-2 text-sm text-white outline-none" />
                </label>
                <label className="text-xs font-medium text-blue-200">
                  备注
                  <input name="notes" className="mt-1 w-full rounded-xl border border-blue-800/40 bg-blue-950/60 px-3 py-2 text-sm text-white outline-none" placeholder="可留空" />
                </label>
              </div>
              <button className="mt-4 rounded-2xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-blue-700">
                新建任务
              </button>
            </form>
            ...
            <div className="mt-4 rounded-2xl border border-blue-800/40 bg-blue-900/20 p-3 text-xs text-blue-200">
              <div className="flex flex-wrap items-center gap-2">
                <span className="rounded-full bg-blue-800/40 px-3 py-1 font-semibold text-white">阻塞任务 {filterCounts.blocked}</span>
                ...
                return (
                  <a
                    ...
                    className={`rounded-full border px-3 py-1.5 text-xs font-semibold transition ${
                      active ? "border-blue-600 bg-blue-600 text-white" : "border-blue-800/40 bg-blue-900/20 text-blue-200 hover:bg-blue-800/30"
                    }`}
                  >
          ...
          <div className="mt-4 grid gap-4 xl:grid-cols-[1.05fr_0.95fr]">
            <DashboardCard>
              <DashboardCardTitle
                title="当前任务"
                desc="按状态筛选后的正式任务列表"
                right={<span className="rounded-full bg-blue-950 px-3 py-1 text-xs font-semibold text-white">{visibleTasks.length} / {tasks.length} 条</span>}
              />
              ...
              <div className="mt-4 grid gap-3">
                {visibleTasks.length > 0 ? visibleTasks.map((task) => {
                  ...
                  return (
                    <div key={task.id} className="rounded-2xl border border-blue-800/40 bg-blue-900/30 p-4">
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        <div>
                          <p className="text-sm font-semibold text-blue-100">{task.id} · {task.title}</p>
                          <p className="mt-1 text-xs text-blue-300">负责人：{task.owner}</p>
                        </div>
                      ...
                      <p className="mt-3 text-sm leading-6 text-blue-300">来源：{task.source}</p>
                      <p className="mt-1 text-sm leading-6 text-blue-300">下一步：{task.nextAction}</p>
                      {task.notes && task.notes !== "-" ? <p className="mt-1 text-sm leading-6 text-blue-300">备注：{task.notes}</p> : null}
                      <p className="mt-1 text-xs text-blue-300">创建时间：{formatEasternTime(task.createdAt)}</p>
                      <p className="mt-1 text-xs text-blue-300">最近更新：{formatEasternTime(task.updatedAt)}</p>
                      {task.blockedBy && task.blockedBy !== "-" ? (
                        task.status === "blocked" ? (
                          <p className="mt-2 text-sm leading-6 text-rose-700">阻塞原因：{task.blockedBy}</p>
                        ) : (
                          <div className="mt-3 rounded-2xl border border-blue-800/40 bg-blue-900/20 p-3">
                            <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-blue-300">系统备注</p>
                            <p className="mt-2 text-sm leading-6 text-blue-300">{task.blockedBy}</p>
                          </div>
                        )
                      ) : null}
                      <div className="mt-3 rounded-2xl border border-blue-800/40 bg-blue-900/20 p-3">
                        <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-blue-300">最近动作 · {relatedEvents.length} 条</p>
                        {relatedEvents.length > 0 ? (
                          <div className="mt-2 space-y-2">
                            {relatedEvents.map((event, index) => (
                              <div key={`${event.stamp}-${index}`} className="rounded-xl bg-blue-900/30 px-3 py-2 text-xs text-blue-300">
                                <span className="font-medium text-blue-200">{displayTaskEventType(event.type)}</span>
                                <span className="mx-2 text-blue-300/70">·</span>
                                <span>{event.result}</span>
                                <span className="mx-2 text-blue-200">·</span>
                                <span>{event.actor}</span>
                                <span className="mx-2 text-blue-200">·</span>
                                <span className="text-blue-300/70">{event.stamp}</span>
                              </div>
                            ))}
                          </div>
                        ) : (
                          <div className="mt-2 rounded-xl bg-blue-900/30 px-3 py-2 text-xs text-blue-300">暂无最近动作</div>
                        )}
                      </div>

                      <form action="/api/task-owner" method="POST" className="mt-3 flex flex-wrap items-center gap-2 rounded-2xl border border-blue-800/40 bg-blue-900/20 p-3">
                        <input type="hidden" name="taskId" value={task.id} />
                        <input type="hidden" name="returnTo" value={`/dashboard/tasks?taskFilter=${taskFilter}`} />
                        <span className="text-xs font-semibold text-blue-300">负责人</span>
                        <select name="owner" defaultValue={task.owner} className="rounded-xl border border-blue-800/40 bg-blue-900/30 px-3 py-1.5 text-xs font-medium text-blue-200 outline-none">
                          {["阿三", "阿本", "小四", "西风", "零号", "待定"].map((owner) => (
                            <option key={owner} value={owner}>{owner}</option>
                          ))}
                        </select>
                        <button type="submit" className="rounded-full border border-blue-800/40 bg-blue-900/30 px-3 py-1.5 text-xs font-medium text-blue-200 transition hover:bg-blue-700">
                          更新负责人
                        </button>
                      </form>
                            ...
                            <button
                              ...
                              className={`rounded-full border px-3 py-1.5 text-xs font-medium transition ${
                                isActive
                                  ...
                                  : "border-slate-200 bg-white text-blue-200 hover:bg-blue-700"
                              }`}
                            >
                      ...
                      <form action="/api/task-meta" method="POST" className="mt-3 rounded-2xl border border-blue-800/40 bg-blue-900/20 p-3">
                        <input type="hidden" name="taskId" value={task.id} />
                        <input type="hidden" name="returnTo" value={`/dashboard/tasks?taskFilter=${taskFilter}`} />
                        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                          <label className="text-xs font-medium text-blue-300">
                            ...
                            <input
                              ...
                              className="mt-1 w-full rounded-xl border border-blue-800/40 bg-blue-900/30 px-3 py-2 text-xs text-blue-200 outline-none"
                            />
                          </label>
                          <label className="text-xs font-medium text-blue-300">
                            阻塞说明
                          ...
                          <label className="text-xs font-medium text-blue-300 xl:col-span-1 sm:col-span-2">
                            备注
                            <input
                              name="notes"
                        ...
                        <button type="submit" className="mt-3 rounded-full border border-blue-800/40 bg-blue-900/30 px-3 py-1.5 text-xs font-medium text-blue-200 transition hover:bg-blue-700">
                          更新任务说明
                        </button>
                      </form>
                    </div>
                  ...
                  <div className="rounded-2xl border border-dashed border-blue-800/40 bg-blue-900/30 px-4 py-6 text-sm text-blue-300">{emptyStateLabel}</div>
                )}
              </div>
            </DashboardCard>
            ...
            <section className="space-y-4">
              <DashboardCard>
                <DashboardCardTitle
                  ...
                  right={<span className="rounded-full bg-blue-950 px-3 py-1 text-xs font-semibold text-white">{activeTodos.length} 条</span>}
                />
                <div className="mt-4 grid gap-3">
                    ...
                    return (
                      <div key={todo.id} className="rounded-2xl border border-blue-800/40 bg-blue-900/30 p-4">
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          ...
                          <span className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-blue-200">已挂任务 {linkedCount}</span>
                        </div>
                        ...
                        <form action="/api/task-create" method="POST" className="mt-3">
                          ...
                          <input type="hidden" name="returnTo" value={`/dashboard/tasks?taskFilter=${taskFilter}`} />
                          <button type="submit" className="rounded-full border border-blue-800/40 bg-blue-900/20 px-3 py-1.5 text-xs font-medium text-blue-200 transition hover:bg-blue-700">
                            提升成正式任务
                          </button>
                        </form>
                      </div>
              ...
              <DashboardCard>
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  <a href="/dashboard/overview" className="rounded-2xl border border-blue-800/40 bg-blue-900/30 p-4 text-sm font-semibold text-blue-100">回总览页</a>
                  <a href="/dashboard/system" className="rounded-2xl border border-blue-800/40 bg-blue-900/30 p-4 text-sm font-semibold text-blue-100">看系统页</a>
                </div>
              </DashboardCard>
            </section>
          </div>
