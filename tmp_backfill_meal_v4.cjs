...
(async () => {
  ...
  // Step 1: Get all T1300 records with C7=1, grouped by employee+date
  const [t1300meals] = await pool.execute(
    "SELECT DISTINCT CAST(P2 AS CHAR) as emp_id, C2 as raw_date FROM db_zhty202410.T1300 WHERE Z1=1 AND C7=1"
  );
  console.log('T1300 C7=1 unique emp+date:', t1300meals.length);

  // Build lookup: emp_id → Set of dates
  ...
  for (const r of t1300meals) {
    ...
    const list = mealLookup.get(r.emp_id) || new Set();
    list.add(date);
    mealLookup.set(r.emp_id, list);
  }

  // Step 2: Get all a3s_attendances with meal=0
  const [attendance] = await pool.execute("SELECT id, date, employee_id FROM a3s_attendances WHERE meal_allowance = 0");
  ...
  for (const a of attendance) {
    const empDates = mealLookup.get(a.employee_id);
  ...
  // Need to get consolidated view since there may be duplicates
  ...
  // Overall stats: how many unique employee+date days have meal?
